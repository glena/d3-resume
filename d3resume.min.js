...
1)}).on("mouseout",function(b){a.selectAll("path.item").transition().attr("stroke-width","2").attr("fill-opacity",0.5);l=setTimeout(u,3E3)})},u=function(){g.selectAll("g.info").transition().attr("fill-opacity",0)},g=d3.select(d.wrapperSelector).append("svg").attr("width",d.width).attr("height",d.height),h=d3.time.scale().range([20,d.width-20]),q=d3.scale.linear().range([d.height,0]);q.domain([0,d.height]);r=d3.svg.axis().scale(h).orient("bottom").ticks(30).tickFormat(d3.time.format("%Y-%m"));d3.json(d.dataUrl,
